...
name: upgrade
description: |
  Self-upgrade upstack by pulling the latest version from the git repo.
...
1. Check if `~/.upstack` exists and is a git repo.
2. If not: follow symlinks from `~/.claude/skills/advisor` to find the repo root.
3. If neither works: report "upstack not installed via git clone. Re-clone to upgrade."
